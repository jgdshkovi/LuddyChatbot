...
# Directories for Level 3
pages_dir = "scraped_pages_lvl3"
links_dir = "scraped_links_lvl3"
os.makedirs(pages_dir, exist_ok=True)
os.makedirs(links_dir, exist_ok=True)

# Input file from Level 2
discovered_links_file = "discovered_links_lvl2.txt"
with open(discovered_links_file, "r", encoding="utf-8") as f:
    lvl3_urls = [line.strip() for line in f if line.strip()]
...
# Scrape each Level 3 URL
for url in lvl3_urls:
    if url in scraped_links:
        print(f"🔁 Already scraped: {url}")
        continue

    print(f"🔍 Scraping Level 3 URL: {url}")
...
# Save Level 4 input
if new_links_to_add:
    with open("discovered_links_lvl3.txt", "w", encoding="utf-8") as f:
        for link in sorted(new_links_to_add):
            f.write(link + "\n")
    print(f"📄 discovered_links_lvl3.txt created with new links.")
